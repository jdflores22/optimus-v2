import { ChangeEvent, useEffect, useMemo, useRef } from 'react';
import { Avatar, Box, Button, Stack, Typography } from '@mui/material';
import PhotoCameraOutlinedIcon from '@mui/icons-material/PhotoCameraOutlined';
import { resolveTerminalLogoUrl } from '../../shared/terminalLogoUtils';

type TerminalLogoFieldProps = {
  name?: string;
  terminalType?: string;
  existingLogoPath?: string | null;
  previewUrl?: string | null;
  onFileSelect: (file: File | null) => void;
  onClear?: () => void;
  error?: string | null;
};

export function TerminalLogoField({
  name,
  terminalType,
  existingLogoPath,
  previewUrl,
  onFileSelect,
  onClear,
  error,
}: TerminalLogoFieldProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const displayUrl = previewUrl ?? resolveTerminalLogoUrl(existingLogoPath);
  const initials = useMemo(() => {
    const source = name?.trim() || (terminalType === 'PortTerminal' ? 'PT' : 'CY');
    const parts = source.split(/\s+/).filter(Boolean);
    if (parts.length >= 2) return `${parts[0][0]}${parts[1][0]}`.toUpperCase();
    return source.slice(0, 2).toUpperCase();
  }, [name, terminalType]);

  useEffect(
    () => () => {
      if (previewUrl?.startsWith('blob:')) {
        URL.revokeObjectURL(previewUrl);
      }
    },
    [previewUrl],
  );

  const onChooseFile = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0] ?? null;
    onFileSelect(file);
    event.target.value = '';
  };

  return (
    <Box
      sx={{
        p: 2,
        borderRadius: 2,
        border: 1,
        borderColor: error ? 'error.main' : 'divider',
        bgcolor: 'action.hover',
      }}
    >
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }}>
        <Avatar
          src={displayUrl}
          variant="rounded"
          sx={{
            width: 72,
            height: 72,
            fontWeight: 700,
            bgcolor: 'background.paper',
            border: 1,
            borderColor: 'divider',
          }}
        >
          {initials}
        </Avatar>
        <Box flex={1} minWidth={0}>
          <Typography variant="subtitle2" fontWeight={700}>
            Terminal / CY logo
          </Typography>
          <Typography variant="caption" color="text.secondary" display="block" mt={0.35} mb={1.25}>
            Upload a square logo for quick recognition in lists, manifests, and yard workflows.
          </Typography>
          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
            <Button
              size="small"
              variant="outlined"
              startIcon={<PhotoCameraOutlinedIcon />}
              onClick={() => inputRef.current?.click()}
            >
              {displayUrl ? 'Change logo' : 'Upload logo'}
            </Button>
            {(displayUrl || existingLogoPath) && onClear && (
              <Button size="small" color="inherit" onClick={onClear}>
                Remove
              </Button>
            )}
          </Stack>
          {error && (
            <Typography variant="caption" color="error.main" display="block" mt={0.75}>
              {error}
            </Typography>
          )}
        </Box>
      </Stack>
      <input
        ref={inputRef}
        type="file"
        hidden
        accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml"
        onChange={onChooseFile}
      />
    </Box>
  );
}
